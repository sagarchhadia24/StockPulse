...
interface UndervaluedContentProps {
  stocks: StockWithScore[];
}

function UndervaluedContentInner({ stocks }: UndervaluedContentProps) {
  ...
  const updateParams = (sector: string, page: number) => {
    ...
    router.push(queryString ? `?${queryString}` : "/undervalued", { scroll: false });
  };
...
export function UndervaluedContent({ stocks }: UndervaluedContentProps) {
  return (
    <Suspense fallback={<StockTable stocks={stocks.slice(0, ITEMS_PER_PAGE)} />}>
      <UndervaluedContentInner stocks={stocks} />
    </Suspense>
  );
}
